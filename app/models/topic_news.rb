class TopicNews < ActiveRecord::Base
  ...
  attr_accessible :id, :title, :sub_title, :body, 
                  :source, :abstract, :user_id, :priority, 
                   :deleted, :created_at, :updated_at, :show_time, :topic_news_images , :topic_news_images_attributes
                   
  has_many :topic_news_images, :dependent => :destroy
  accepts_nested_attributes_for :topic_news_images, :reject_if => lambda { |a| a[:image].blank? }, :allow_destroy => true
  belongs_to :topic
